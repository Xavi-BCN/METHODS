...
function getAllDirectors(array) {
  ...
  const allDirectors = array.map(films => films.director);
  /* alert(`Aqui están los directores:
  ${JSON.stringify(allDirectors)}`) */
  alert(allDirectors)
...
function getMoviesFromDirector(array, choiseDirector) {
  const filmsOfDirector = array.filter( films => films.director === choiseDirector);
  alert(`Aqui están las peliculas de : ${choiseDirector}
  ${JSON.stringify(filmsOfDirector)}`)
...
function moviesAverageOfDirector(array, choiseDirector) {
  let average = 0;
  let totalScore = 0;
  let foundFilms = 0;
  totalScore = array.reduce((acc, film) => {
    if(film.director === choiseDirector){
      acc += film.score;
      foundFilms ++;
    }
    return acc
  },0);
  average = totalScore / foundFilms
  alert(`The average of Quentin is: ${parseFloat(average).toFixed(2)}`)
  return average;
}

// Exercise 4:  Alphabetic order by title 
function orderAlphabetically(array) {
const ordenedListFilms = array.map(({...film}) => film = film.title)
.sort()
alert(ordenedListFilms);
return ordenedListFilms;
